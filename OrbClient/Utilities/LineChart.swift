    ...
    */
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        ...
        for index in 0..<data.count {
            let xValue = self.x.scale(CGFloat(index)) + x.axis.inset - dots.outerRadius/2 + 0
            let yValue = self.bounds.height - self.y.scale(data[index]) - y.axis.inset - dots.outerRadius/2
            
            // draw custom layer with another layer in the center
            let dotLayer = DotCALayer()
            dotLayer.dotInnerColor = UIColor.white//colors[lineIndex]
            ...
            if data[index] > 0
            {
                let lblValue = UILabel(frame: CGRect(x: xValue+dots.outerRadius, y: yValue, width: 30, height: 15))
                lblValue.backgroundColor = UIColor.clear
                lblValue.text = String(describing: Int(data[index])) + "K"
    ...
    fileprivate func drawAxes() {
        ...
        path.move(to: CGPoint(x: x.axis.inset + 0, y: y0))
        path.addLine(to: CGPoint(x: width - x.axis.inset + 0, y: y0))
        path.stroke()
        // draw y-axis
        y.axis.color.setStroke()
        path.move(to: CGPoint(x: x.axis.inset + 0, y: height - y.axis.inset))
        path.addLine(to: CGPoint(x: x.axis.inset + 0, y: y.axis.inset))
        path.stroke()
    }
    ...
    fileprivate func drawLine(_ lineIndex: Int) {
        ...
        layer.strokeColor =  UIColor(red: 255.0/255.0, green: 23.0/255.0, blue: 83.0/255.0, alpha: 1.0).cgColor//colors[lineIndex].cgColor
        layer.fillColor = nil
    ...
    fileprivate func drawXLabels() {
        ...
        for (index, _) in xAxisData.enumerated() {
            let xValue = self.x.scale(CGFloat(index)) + x.axis.inset - (width / 2) + 0
