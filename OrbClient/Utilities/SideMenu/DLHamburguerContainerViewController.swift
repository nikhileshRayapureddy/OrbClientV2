...
class DLHamburguerContainerViewController: UIViewController {
    // structure
    weak var hamburguerViewController: SideMenu!      // root hamburguer view controller
    ...
    override func viewDidLoad() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureRecognized(_:)))
                backgroundFadingView.addGestureRecognizer(tapGesture)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if !hamburguerViewController.menuVisible {
            ...
            case .Left:
                self.setContainerFrame(frame: CGRect(x:-self.hamburguerViewController.actualMenuViewSize.width,y: 0, width:self.hamburguerViewController.actualMenuViewSize.width,height: self.hamburguerViewController.actualMenuViewSize.height))
            case .Right:
                self.setContainerFrame(frame: CGRect(x:self.view.frame.size.width,y: 0, width:self.hamburguerViewController.actualMenuViewSize.width,height: self.hamburguerViewController.actualMenuViewSize.height))
            case .Top:
                self.setContainerFrame(frame: CGRect(x:0, y: -self.hamburguerViewController.actualMenuViewSize.height, width: self.hamburguerViewController.actualMenuViewSize.width, height:self.hamburguerViewController.actualMenuViewSize.height))
            case .Bottom:
                self.setContainerFrame(frame: CGRect(x :0, y:self.view.frame.size.height,width: self.hamburguerViewController.actualMenuViewSize.width, height:self.hamburguerViewController.actualMenuViewSize.height))
            }
        }
    ...
    func resizeToSize(size: CGSize) {
        ...
        UIView.animate(withDuration: self.hamburguerViewController.animationDuration, animations: { () -> Void in
            self.setContainerFrame(frame: newFrame)
    ...
    func show() {
        ...
        UIView.animate(withDuration: self.hamburguerViewController.animationDuration, animations: { () -> Void in
            self.setContainerFrame(frame: finalFrame)
            self.backgroundFadingView.alpha = self.hamburguerViewController.overlayAlpha
        }) { (success) -> Void in
            // inform the delegate.
            if self.hamburguerViewController.delegate != nil {
               ...
               self.hamburguerViewController.delegate?.hamburguerViewController!(hamburguerViewController: self.hamburguerViewController, didShowMenuViewController: self.hamburguerViewController.menuViewController)
    ...
    /** Hides the menu. */
    func hide() {
        hideWithCompletion(completion: nil)
    }
    ...
    func hideWithCompletion(completion: ((Void) -> Void)?) {
        ...
        self.hamburguerViewController.delegate?.hamburguerViewController!(hamburguerViewController: self.hamburguerViewController, willHideMenuViewController: self.hamburguerViewController.menuViewController)
        ...
        UIView.animate(withDuration: self.hamburguerViewController.animationDuration, animations: { () -> Void in
            ...
            self.hamburguerViewController.hamburguerHideController(controller: self)
            
            
            self.hamburguerViewController.delegate?.hamburguerViewController!(hamburguerViewController: self.hamburguerViewController, didHideMenuViewController: self.hamburguerViewController.menuViewController)
            completion?()
        }
    }
    ...
    func panGestureRecognized(recognizer: UIPanGestureRecognizer) {
        ...
        self.hamburguerViewController.delegate?.hamburguerViewController!(hamburguerViewController: self.hamburguerViewController, didPerformPanGesture: recognizer)
        ...
        // changed: adjust frame
            ...
            self.setContainerFrame(frame: frame)
        }
    ...
    func fixLayoutWithDuration(duration: TimeInterval) {
        var newFrame = CGRect.zero
        switch (hamburguerViewController.menuDirection) {
        case .Left:
    ...
    override func willAnimateRotation(to toInterfaceOrientation: UIInterfaceOrientation, duration: TimeInterval) {
        ...
        self.fixLayoutWithDuration(duration: duration)
    }
    ...
    // iOS 8 Transition.
     func viewWillTransitionToSize(size: CGSize, withTransitionCoordinator coordinator: UIViewControllerTransitionCoordinator) {
        ...
        self.fixLayoutWithDuration(duration: coordinator.transitionDuration)
    }
